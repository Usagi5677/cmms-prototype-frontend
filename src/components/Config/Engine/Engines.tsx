...
import { DELETE_ENGINE, DELETE_HULL_TYPE } from "../../../api/mutations";
...
export const Engines: React.FC<EngineProps> = ({}) => {
  ...
  const [getEngines, { data, loading }] = useLazyQuery(ENGINES, {
    onError: (err) => {
      errorMessage(err, "Error loading engines.");
    },
  ...
  const pageInfo = data?.engines.pageInfo ?? {};
  ...
  return (
    <div>
      ...
      <Table
        rowKey="id"
        dataSource={data?.engines.edges.map(
          (edge: { node: Engine }) => edge.node
        )}
        columns={columns}
        pagination={false}
        size="small"
