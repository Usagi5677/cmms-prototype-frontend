...
import { ASSIGN_USER_TO_MACHINE } from "../../../api/mutations";
import { GET_USERS_WITH_PERMISSION } from "../../../api/queries";
import { errorMessage } from "../../../helpers/gql";
import { UserTagStringToColor } from "../../../helpers/style";
import User from "../../../models/User";
import classes from "./MachineAssignment.module.css";

const MachineAssignment = ({ machineID }: { machineID: number }) => {
  const [visible, setVisible] = useState(false);
  const [form] = useForm();
  const [assignUserToMachine, { loading: loadingAssign }] = useMutation(
  ...
  useEffect(() => {
    getUsersWithPermission({
      variables: { permissions: ["ADD_MACHINE_REPAIR"] },
    });
  }, [getUsersWithPermission]);
