...
import classes from "./TransportationUtilizationGraph.module.css";
import { GET_ALL_TRANSPORTATION_USAGE_HISTORY } from "../../../../api/queries";
...
const TransportationUtilizationGraph = () => {
  const { id }: any = useParams();
  const [dates, setDates] = useState<any>([
    moment().subtract(1, "day"),
    moment(),
  ]);
  const [allTransportationUsageHistory, { data: history, loading }] =
    useLazyQuery(GET_ALL_TRANSPORTATION_USAGE_HISTORY, {
      onError: (err) => {
  ...
  }, [dates, allTransportationUsageHistory]);
  ...
  let data = () => {
    let labels = history?.allTransportationUsageHistory.map((rec: any) =>
    ...
    let datasets = labelData.map((label) => {
      ...
      return {
        ...
        borderColor: color,
        borderWidth: 1,
        data: history?.allTransportationUsageHistory.map((rec: any) => {
...
export default TransportationUtilizationGraph;
